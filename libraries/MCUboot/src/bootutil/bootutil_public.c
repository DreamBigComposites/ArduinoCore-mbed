...
boot_write_magic(const struct flash_area *fap)
{
    ...
    MCUBOOT_LOG_DBG("writing magic; fa_id=%d off=0x%lx (0x%lx)",
...
boot_write_image_ok(const struct flash_area *fap)
{
    ...
    MCUBOOT_LOG_DBG("writing image_ok; fa_id=%d off=0x%lx (0x%lx)",
                 flash_area_get_id(fap), (unsigned long)off,
                 (unsigned long)(flash_area_get_off(fap) + off));
                     ...
                     uint8_t image_num)
{
    ...
    off = boot_swap_info_off(fap);
    MCUBOOT_LOG_DBG("writing swap_info; fa_id=%d off=0x%lx (0x%lx), swap_type=0x%x"
                 " image_num=0x%x",
                 flash_area_get_id(fap), (unsigned long)off,
                 (unsigned long)(flash_area_get_off(fap) + off),
                 swap_type, image_num);
...
boot_swap_type_multi(int image_index)
{
    ...
    rc = boot_read_swap_state_by_id(FLASH_AREA_IMAGE_SECONDARY(image_index),
                                    &secondary_slot);
    if (rc == BOOT_EFLASH) {
        MCUBOOT_LOG_INF("Secondary image of image pair (%d.) "
